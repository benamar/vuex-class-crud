...
export class Entity implements IObject {
  ...
  async getHeaders() {
    const routeHeaders = typeof this.routeHeaders === 'function' ? await this.routeHeaders(this) : this.routeHeaders;
    const apiHeaders = typeof this.apiHeaders === 'function' ? await this.apiHeaders(this) : this.apiHeaders;
  ...
  constructor(apiConfig: IApiRouteConfig,  name: string) {
    ...
    this.afterRequest = routeConfig.hooks && routeConfig.hooks.afterRequest || this.afterRequest;
    this.beforeRequest = apiConf && apiConf.hooks && apiConf.hooks.beforeRequest;
    this.beforeRequest = routeConfig.hooks && routeConfig.hooks.beforeRequest || this.beforeRequest;
  ...
  public async create(content: IVRequestOptions): Promise<object> {
     try {
  ...
  public async put(content: IVRequestOptions): Promise<object> {
    try {
      // console.log("creating  newElement params", content.params);
      let { params, query, errorCallback, headers, body: data } = content;
      data = data || content;
  ...
  public async patch(content: IVRequestOptions): Promise<object> {
    try {
      // console.log("creating  newElement params", content.params);
      let { params, query, errorCallback, headers, body: data } = content;
      data = data || content;
  ...
  public async remove(content: IVRequestOptions): Promise<object> {
    try {
